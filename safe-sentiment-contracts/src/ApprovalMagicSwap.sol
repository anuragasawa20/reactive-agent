...
contract ApprovalMagicSwap is IApprovalClient {
    address private constant ROUTER_ADDR =
    ...
    constructor(address _service, address _token1, address _token2) {
        owner = payable(msg.sender);
        service = ApprovalService(_service);
    ...
    function subscribe() external onlyOwner {
        ...
        require(
            subscription_fee <= address(this).balance,
    ...
    function onApproval(
        ...
        require(
            approved_token == address(token0) ||
                approved_token == address(token1),
            "Token not supported"
        );
        require(
            amount == IERC20(approved_token).allowance(approver, address(this)),
            "Approved amount mismatch"
        );
        require(
            amount <= IERC20(approved_token).balanceOf(approver),
            "Insufficient tokens"
        );
        assert(
            IERC20(approved_token).transferFrom(approver, address(this), amount)
        );
        assert(IERC20(approved_token).approve(address(router), amount));
        address[] memory path = new address[](2);
        path[0] = approved_token;
        path[1] = approved_token == address(token0)
            ? address(token1)
            : address(token0);
    ...
    function settle(uint256 amount) external onlyService {
        require(
